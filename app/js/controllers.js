 ...
 **********************************************************************/controller('NavCtrl', function($rootScope, $scope, AuthService) {
	...
	$scope.login = function() {
		AuthService.login($scope.user.username, $scope.user.password).then(function(user) {
 ...
 */controller('SignupCtrl', function($scope, $rootScope, $location, AuthService) {
	...
	$scope.signup = function() {
		...
		var username = $scope.user.username, email = $scope.user.email, fullname = $scope.user.fullname, password = $scope.user.password;

		AuthService.signup(username, email, fullname, password).success(function(user) {
	...
	function() {
		...
		FileService.
		delete (filesToDelete).then(function() {
			...
			angular.each($scope.files, function(file) {
				if (_.contains(filesToDelete, file.id)) {
					$scope.files = _.without($scope.files, _.findWhere($scope.files, {
						id : file.id
					}));
				}
			});
